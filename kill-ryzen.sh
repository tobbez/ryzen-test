...
wget -q -O "$DOWNLOAD_FILE" -c ftp://ftp.fu-berlin.de/unix/languages/gcc/releases/gcc-7.2.0/$DOWNLOAD_FILE || exit 1
...
pushd / >/dev/null
  stdbuf -eL -oL journalctl -kf -o short-iso | sed 's/^/[KERN] /' &
popd >/dev/null
